 ...
 *
 * 検索の入力欄はこのバーではなくページ側 (/songs の LiveSearch) にある。
 * 一度ここへ移して App Store と同じ「検索タブではタブバーが検索欄に化ける」
 * 形にしたが、入力欄が画面下端に来ると、キーボードの直上にブラウザ自前の
 * アクセサリバー (Chrome for iOS のパスワード / カード / 住所ボタン) が
 * 並んでしまう。あれはページ側から消せないので、下端に入力欄を置く限り
 * 体験が破綻する。それで元の「ページ上部の入力欄」に戻した。
 * 再挑戦するならまずそのアクセサリバーの回避策から。
 */
                  ...
                  <li key={item.href} className="min-w-0">
                    ...
                    >
                      {/* 純正の SF Symbol はグリフ 18pt 前後 (実測)。
                          lucide は viewBox いっぱいに描くので、同じ 24px に
                          するとタブだけ一回り大きく見える。ラベルを持たない
                          円ボタン側は純正も 22-23pt あるので 24px のまま。 */}
