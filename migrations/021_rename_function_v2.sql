-- ============================================================================
-- get_unrated_songs_v2 として完全に新しい関数を作成 (キャッシュ強制バイパス)
-- ============================================================================
-- 実行先: Supabase (PostgreSQL 15+)
-- 前提: 020 まで実行済み
--
-- 背景:
--   020 で DROP→再作成しても、SQL Editor 直接呼出しは正しいが
--   アプリ経由 (PostgREST RPC) は古い結果が返る現象。
--   PostgREST 接続プール内で旧プランが永続キャッシュされていると推測。
--
--   解決: 同じロジックを別関数名 get_unrated_songs_v2 として作成。
--   page.tsx でも .rpc("get_unrated_songs_v2", ...) に変更し、
--   Vercel 再ビルドで強制的に新エンドポイントを叩かせる。
-- ============================================================================
